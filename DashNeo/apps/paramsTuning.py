...
# --------------------------------------
layout = html.Div([
    html.Div(html.H2("Parameters Tuning"), style={"text-align": "center"}),
    html.Hr(),
    ...
    dbc.Row([
            dbc.Col([
                dcc.Input(id="rerun-by-inputId", type="text",
                          placeholder="Enter Input ID", value='',
                          style={'width': '65%', 'marginRight': '20px'}),
                dbc.Button("Confirm Input ID",
                           id="button-confir-inputId", outline=True, color="success", className="me-1"),
                dcc.Store(id='seq-length-store2',
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Div([
                ...
                dcc.Input(id="input_windowSize", type="number", min=1,
                          # max=max(ref_genes_len.values())-1,
                          placeholder="Enter Sliding Window Size", value=9,
                          style={'width': '65%', 'marginRight': '20px'}),
                html.Div(id='input_windowSize-container'),
            ]),
        ...
        dbc.Col([
            html.Div([
                html.H3("Step size"),
                dcc.Input(id="input_stepSize", type="number", min=1,
                          # max=max(ref_genes_len.values())-1,
                          placeholder="Enter Step Size", value=3,
                          style={'width': '65%', 'marginRight': '20px'}),
                html.Div(id='input_stepSize-container'),
            ]),
        ],  # width={'size':3, 'offset':1, 'order':1},
            xs=12, sm=12, md=12, lg=5, xl=5
        ),
        ...
        dbc.Row([
            dbc.Col([
                ...
                html.Div([
                    ...
                    html.Div(id='input-strategy-container2'),
            ...
            dbc.Col([
                html.Div([html.H3("Average Calculation Interval (day)"),
                         dcc.Input(id="average-interval2", type="number", min=3,
                                   # max=max(ref_genes_len.values())-1,
                                   # placeholder="Interval for which the environmental values will be averaged",
                                   value=3,
                                   style={'width': '65%', 'marginRight': '20px'}),
                         html.Div(id='Average-interval-container2'),
                          ]),
            ], xs=12, sm=12, md=12, lg=5, xl=5),
            ...
            dbc.Col([
                dcc.Loading(
                    ...
                    children=[
                        ...
                        dcc.Download(id="download-component-csv3"),
                        html.Br(),
    ...
    dbc.Container([
        ...
        dbc.Row([
                dbc.Col([
                    html.Br(),
                    html.Div(id="analysis-info2"),
                    dbc.Button(id='btn-params-info2',
                               children=[
                    ...
                    dcc.Download(id="download-component-params2"),
...
@app.callback(
    Output('forCSV-table2', 'data'),
    [Input('button-confir-param2', 'n_clicks'),
     ...
     State('strategy-radio2', 'value'),
     State('average-interval2', 'value'),
     State('rerun-by-inputId', 'value')
     ],
)
def update_table(n, bootstrap, rf, window_size, step_size, strategy, average_interval, input_id):
    ...
    if n is None:
        ...
        with open('config/config.yaml', 'r') as file:
            ...
            config['input']['input_name'] = input_id
...
@app.callback(
    Output('success_message2', 'children'),
    Output('btn-params-info2', 'style'),
    Input('button-submit2', 'n_clicks'),
    State(component_id='forCSV-table2',
...
@app.callback(
    Output("download-component-csv3", "data"),
    Input("btn-csv2", "n_clicks"),
    State('forCSV-table2', "derived_virtual_data"),
    prevent_initial_call=True,
)
...
@app.callback(
    Output("download-component-params2", "data"),
    Input("btn-params-info2", "n_clicks"),
    prevent_initial_call=True,
)
def func(n_clicks):
    if n_clicks is None:
...
@app.callback(
    Output('seq-length-store2', 'data'),
    Input('button-confir-inputId', 'n_clicks'),
    Input('rerun-by-inputId', 'value'),
)
def retrieve_config_values(n, input_id):
    if n is None:
        return None
    else:
        seq_list, length_min = neoCypher_manager.get_seqByInputName(input_id)
        print(length_min)
        with open('config/config.yaml', 'r') as file:
            config = yaml.safe_load(file)

        config['seqinfo']['accession_lt'] = seq_list

        if length_min > 25000:
            config['params']['data_type'] == 'dna'
        else:
            config['params']['data_type'] == 'aa'
...
@app.callback(
    dash.dependencies.Output(
        'Average-interval-container2', 'children'),
    [dash.dependencies.Input('average-interval2', 'value')])
...
@app.callback(
    dash.dependencies.Output(
        'BootstrapThreshold-slider-output-container', 'children'),
...
@app.callback(
    dash.dependencies.Output(
        'RFThreshold-slider-output-container', 'children'),
...
# Once user click the 'Comfirm Input ID' button
# (1) sequence list in the config.yaml file calculate the minimum lengh of seq


@app.callback(
    dash.dependencies.Output('input_windowSize-container', 'children'),
    [dash.dependencies.Input('input_stepSize', 'value'),
     Input('seq-length-store2', 'data'),
     ])
...
@app.callback(
    dash.dependencies.Output('input_stepSize-container', 'children'),
